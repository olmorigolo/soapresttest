...
public class SoapServiceClientTest {
	...
	public final void testUnsecuredSoap() {
		...
		assertEquals("PromoTest promotion", response.evalValue("//ns2:name"));
	}
...
//		}
		...
		for (Service srv : wsdl.getServices()) {
			...
			for (Port op : srv.getPorts()) {
				System.out.println(" -portTypeName:" + op.getName() + " adress:" + op.getAddress().getLocation() + " bindingname:" + op.getBinding().getName());
			}
		}
